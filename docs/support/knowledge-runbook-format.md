...
`# Objective`, `## Safe evidence`, `## Non-executable guidance`, and
`## Stop and escalate`. Missing, empty, duplicate, reordered, or additional
headings invalidate the corpus. No credentials or personal data are permitted.
...
unsupported document invalidates the request without returning partial results.
Only direct, regular `.md` children are considered; returned evidence contains
only a file name relative to the root.
