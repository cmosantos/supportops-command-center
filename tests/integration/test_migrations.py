...
def test_status_nonexistent_is_non_mutating_and_init_is_idempotent(
    ...
    assert runner.status() == (0, (1, 2, 3, 4))
    assert not path.exists()
    assert runner.apply("2026-08-01T00:00:00+00:00") == 4
    assert runner.apply("2026-08-01T00:00:01+00:00") == 0
    assert runner.status() == (4, ())
...
def test_existing_v1_database_receives_only_v2_in_order(tmp_path: Path) -> None:
    ...
    assert full.status() == (1, (2, 3, 4))
    assert full.apply("2026-08-01T00:00:01+00:00") == 3
    assert full.status() == (4, ())
